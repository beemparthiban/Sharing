...
public class AddObjetoActivity extends AppCompatActivity{
    ...
    private  void cadastrarObjeto() throws ParseException {
        if (validateFieldsEvento()) {
            try {
                ...
                sessaoUsuario.getPessoaLogada().addPontuacao(30);
                GuiUtil.exibirMsg(this, getString(R.string.adc_objeto_sucesso));
                startPerfilActivity();
            } catch (Exception e) {
                GuiUtil.exibirMsg(AddObjetoActivity.this, e.getMessage());
    ...
    public void tirarFotoObj(View v){
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            String nomeFoto = android.text.format.DateFormat.format("yyyy-MM-dd_hhmmss.png", new Date()).toString();
    ...
    public static final Uri FOTO_PADRAO = Uri.parse("android.resource://br.com.sharing/"+R.drawable.object_default);

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        if(resultCode == RESULT_OK && requestCode == TIRA_FOTO && ajustarFoto() != null){
            foto = GuiUtil.getInstancia().getImageUri(AddObjetoActivity.this , ajustarFoto());
        }else{
            foto = FOTO_PADRAO;
        }
    }
